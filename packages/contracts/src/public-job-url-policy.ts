...
export function isPublicDnsHostname(value: string): boolean {
  ...
  return labels.length >= 2
    ...
    && !["localhost", "local", "internal", "lan"].some((suffix) => value === suffix || value.endsWith(`.${suffix}`))
    && getDomain(value) !== null;
}

export const SafeNormalizedPublicJobUrlSchema = z.url().max(2_048).superRefine((value, context) => {
  const url = new URL(value);
  const hasUnsafeAuthority = url.protocol !== "https:" || url.username !== "" || url.password !== "" || url.hash !== "" || !isPublicDnsHostname(url.hostname);
