...
import {
  jobOpportunities, jobProfiles, jobSourcePostingVersions, jobTargetRevisions, jobTargets,
...
async function currentFacts(db: Pick<Database, "select">, userId: string, profileId: string) {
  ...
  )).where(and(eq(profileFacts.userId, userId), eq(profileFacts.profileId, profileId)))
  ...
  return {
    async create(input) {
      return deps.db.transaction(async (transaction) => {
        ...
        const facts = await currentFacts(transaction, input.userId, profile.id);
        if (!facts.length) throw new JobTriageError("JOB_TRIAGE_PROFILE_EMPTY");
        const existing = await findVersion(transaction, { userId: input.userId, opportunityId: opportunity.id, sourcePostingVersionId: opportunity.sourcePostingVersionId, profileId: profile.id, profileVersion: profile.version, targetId: target.id, targetVersion: target.version });
        ...
        const triage = evaluateJobTriage({
          sourcePostingVersionId: opportunity.sourcePostingVersionId, now: deps.clock(),
          target: { targetId: target.id, version: target.version, constraints: JobTargetConstraintsSchema.parse(target.constraints) },
        ...
        await deps.auditTrail.bind(transaction).append({
          userId: input.userId, actorUserId: input.userId, eventType: "job.triage_created", occurredAt: deps.clock(), requestId: input.requestId,
          outcome: "success", reasonCode: "JOB_TRIAGE_CREATED", resourceType: "job_triage_version", resourceId: created.id,
          metadata: { triageVersionId: created.id, opportunityId: opportunity.id, sourcePostingVersionId: opportunity.sourcePostingVersionId, profileId: profile.id, profileVersion: profile.version, targetId: target.id, targetVersion: target.version, qualificationRuleVersion: QUALIFICATION_RULE_VERSION, coarseRuleVersion: COARSE_RULE_VERSION, overallVerdict: triage.overallVerdict, deadlineStatus: triage.deadlineStatus },
        });
