...
function deterministicUuid(fingerprint: string): string {
...
function queryText(input: {
  ...
  return [
    ...
    ...input.skills,
    input.companyName,
    ...(input.siteDomains?.map((domain) => `site:${domain}`) ?? []),
  ].filter((value): value is string => value !== null && value !== undefined && value !== "").map(normalizeWhitespace).join(" ");
}
...
export function createAnySearchQueryPlan(input: unknown) {
  ...
  const createQuery = (kind: "general" | "site_constrained" | "target_company", input: {
    ...
    const allowedSiteDomains = input.allowedSiteDomains ? [...input.allowedSiteDomains] : [];
    ...
    return {
      queryId: deterministicUuid(stableFingerprint),
      kind,
      stableFingerprint,
      query,
      allowedSiteDomains,
