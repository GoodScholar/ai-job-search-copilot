...
import { getWorkbenchHome } from "./workbench-home";
...
describe("workbench home", () => {
  ...
  it("returns the real empty workbench only for an active account", async () => {
    await expect(getWorkbenchHome({ db: database, userId: activeUserId })).resolves.toEqual({
  ...
  it("uses the domain account-not-found code for missing or inactive accounts", async () => {
    ...
    await expect(getWorkbenchHome({ db: database, userId: inactiveUserId }))
      .rejects.toMatchObject({ code: "ACCOUNT_NOT_FOUND" });
  });
});
