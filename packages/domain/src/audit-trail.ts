...
const AgentInboxOpenedMetadataSchema = z.discriminatedUnion("kind", [
  ...
  z.object({ occurrenceId: z.uuid(), kind: z.literal("schedule_attention"), reasonCode: z.literal("SCHEDULE_RUN_PREFLIGHT_BLOCKED"), budgetDimension: z.null() }).strict(),
]);
...
const JobDiscoveryScheduleMetadataSchema = z.object({
  ...
  skipReason: z.enum(["TARGET_INACTIVE", "NO_SUPPORTED_SOURCE", "SOURCE_POLICY_REQUIRED", "PROFILE_UNAVAILABLE", "ACCOUNT_RUN_POLICY_WINDOW_CLOSED", "RUN_PREFLIGHT_BLOCKED", "ACCOUNT_RUN_STOPPED", "ACCOUNT_RUN_SCHEDULE_SKIPPED"]).nullable(),
}).strict();
...
const AccountRunControlRejectedMetadataSchema = z.object({ commandId: z.uuid(), action: z.enum(["stop", "release"]), expectedVersion: z.int().nonnegative() }).strict();
...
const AuditEventInputSchema = z.discriminatedUnion("eventType", [
  ...
  z.object({ userId: z.uuid(), actorUserId: z.uuid(), eventType: z.literal("account.run_control_rejected"), occurredAt: z.date().optional(), requestId: z.uuid(), outcome: z.literal("failure"), reasonCode: z.enum(["ACCOUNT_RUN_CONTROL_COMMAND_ID_CONFLICT", "ACCOUNT_RUN_CONTROL_VERSION_CONFLICT"]), resourceType: z.literal("account_run_control"), resourceId: z.uuid(), metadata: AccountRunControlRejectedMetadataSchema }).strict(),
  ...
  z.object({ userId: z.uuid(), actorUserId: z.uuid(), eventType: z.literal("agent.inbox_opened"), occurredAt: z.date().optional(), requestId: z.uuid(), outcome: z.literal("success"), reasonCode: z.union([z.literal("AGENT_RUN_PAUSED"), z.literal("SOURCE_HEALTH_ATTENTION"), z.literal("DISCOVERY_ATTENTION"), z.literal("SCHEDULE_RUN_PREFLIGHT_BLOCKED"), AgentRunFailureCodeSchema]), resourceType: z.literal("agent_inbox_item"), resourceId: z.uuid(), metadata: AgentInboxOpenedMetadataSchema }).strict(),
