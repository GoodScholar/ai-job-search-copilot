...
const AgentInboxActionAppliedMetadataSchema = z.object({
  itemId: z.uuid(), runId: z.uuid(), action: z.enum(["restart_run", "resume_run", "cancel_run", "dismiss"]), outcome: z.enum(["applied", "no_change", "failed"]), reasonCode: z.union([z.literal("AGENT_RUN_PAUSED"), z.literal("AGENT_INBOX_ACTION_FAILED"), AgentRunFailureCodeSchema]),
}).strict();
