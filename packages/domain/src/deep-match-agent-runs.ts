...
export async function ensureDeepMatchRunInTransaction(input: { transaction: any; id: () => string; clock: () => Date; userId: string; targetId: string; idempotencyKey: string; trigger: "automatic" | "manual"; opportunityId?: string }) {
...
export function createDeepMatchRunStarter(deps: { db: Database; queue: DeepMatchRunQueue; id: () => string; clock: () => Date }) {
  return {
    async start(input: { userId: string; targetId: string; opportunityId?: string; idempotencyKey: string; trigger: "automatic" | "manual" }) {
