...
type RuntimeEnvironment = Partial<Record<"APP_ENV" | "PUBLIC_JOB_DISCOVERY_ADAPTER" | "E2E_AGENT_RUN_SCENARIOS" | "E2E_PUBLIC_SOURCE_HEALTH_SCENARIOS" | "ANYSEARCH_BASE_URL" | "ANYSEARCH_PROVIDER_BASE_URL", string | undefined>>;
...
export type JobDiscoveryRuntimeConfig = Readonly<{ environment: "production" | "local" | "test"; executionMode: JobDiscoveryExecutionMode; agentRunScenarios: AgentRunScenarioMap; sourceHealthScenarios: SourceHealthScenarioMap }>;
...
export function resolveJobDiscoveryRuntimeConfig(environment: RuntimeEnvironment): JobDiscoveryRuntimeConfig {
  ...
  if (appEnv !== "test" && (environment.E2E_AGENT_RUN_SCENARIOS !== undefined || environment.E2E_PUBLIC_SOURCE_HEALTH_SCENARIOS !== undefined || configured(environment.ANYSEARCH_BASE_URL) || configured(environment.ANYSEARCH_PROVIDER_BASE_URL))) return invalid();
  ...
  const executionMode = appEnv === "production" ? "layered_public" as const : appEnv === "test" ? (Object.keys(sourceHealthScenarios).length > 0 ? "greenhouse" as const : "fake" as const) : environment.PUBLIC_JOB_DISCOVERY_ADAPTER === "greenhouse" ? "greenhouse" as const : "fake" as const;
  return Object.freeze({ environment: appEnv, executionMode, agentRunScenarios: Object.freeze(agentRunScenarios), sourceHealthScenarios: Object.freeze(sourceHealthScenarios) });
}
