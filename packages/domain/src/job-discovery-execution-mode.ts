...
export function validateJobDiscoveryRuntimeConfig(environment: RuntimeEnvironment): void {
  ...
  if (environment.APP_ENV !== "test" && (environment.E2E_AGENT_RUN_SCENARIOS !== undefined || environment.E2E_PUBLIC_SOURCE_HEALTH_SCENARIOS !== undefined || configured(environment.ANYSEARCH_BASE_URL) || configured(environment.ANYSEARCH_PROVIDER_BASE_URL))) throw new Error("JOB_DISCOVERY_RUNTIME_CONFIG_INVALID");
}
...
export function resolveJobDiscoveryExecutionMode(environment: RuntimeEnvironment): JobDiscoveryExecutionMode {
  ...
  if (environment.APP_ENV === "test") return configured(environment.E2E_PUBLIC_SOURCE_HEALTH_SCENARIOS) ? "greenhouse" : "fake";
