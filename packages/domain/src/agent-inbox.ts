...
export function createAgentInbox(deps: { db: Database; commands: Commands; auditTrail: AuditTrail; id: () => string; clock: () => Date }): AgentInbox {
  ...
  const releasePreflightClaim = async (input: { userId: string; itemId: string; actionId: string }) => deps.db.transaction(async (tx) => {
    await acquireAccountAdvisoryLock(tx, input.userId);
    await tx.delete(agentInboxItemActions).where(and(eq(agentInboxItemActions.userId, input.userId), eq(agentInboxItemActions.itemId, input.itemId), eq(agentInboxItemActions.actionId, input.actionId), eq(agentInboxItemActions.action, "restart_run"), eq(agentInboxItemActions.outcome, "pending")));
  });
