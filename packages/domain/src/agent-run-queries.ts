...
import {
  agentRunEvents, agentRunJobResults, agentRunSteps, agentRuns, jobSourceHealthChecks, jobOpportunities, jobSourcePostings, jobSourcePostingVersions,
  type Database,
...
async function detail(db: Database, userId: string, runId: string): Promise<AgentRunDetail | null> {
  ...
  if (!run) return null;
  const [steps, events, results, sourceChecks] = await Promise.all([
  ...
  return AgentRunDetailSchema.parse({ ...runSummary, executionSpec: { targetSnapshot: run.targetSnapshot, sourceScope, workflowVersion: run.workflowVersion, ruleVersion: run.ruleVersion, adapter: run.adapter, adapterVersion: run.adapterVersion, outputSchemaVersion: run.outputSchemaVersion, toolAllowlist: run.toolAllowlist, model: run.modelSnapshot, budget: run.budgetSnapshot }, controlState: run.controlState, usage, termination, retryOfRunId: run.retryOfRunId, ...(run.workflowVersion === "job-discovery-workflow-v3" ? { sourceChecks: sourceChecks.map((check) => ({ checkId: check.id, runId: check.runId, targetId: check.targetId, watchlistItemId: check.watchlistItemId, sourceId: check.sourceId, status: check.status, reasonCodes: check.reasonCodes, impact: { scope: check.impactScope, affectedCount: check.impactAffectedCount }, observedPostingCount: check.observedPostingCount, selectedDetailCount: check.selectedDetailCount, validDetailCount: check.validDetailCount, requestAttemptCount: check.requestAttemptCount, checkedAt: check.checkedAt.toISOString() })) } : {}), steps: steps.map((step) => ({ stepKey: step.stepKey, ordinal: step.ordinal, status: step.status, attemptCount: step.attemptCount, startedAt: step.startedAt?.toISOString() ?? null, completedAt: step.completedAt?.toISOString() ?? null, failedAt: step.failedAt?.toISOString() ?? null, failureCode: step.failureCode })), events: events.map((event) => ({ sequence: event.sequence, runVersion: event.runVersion, eventType: event.eventType, data: event.data, createdAt: event.createdAt.toISOString() })), results: results.map(({ normalizedData, ...result }) => {
