    ...
    async persistSuccessfulDiscovery(input: {
      ...
      const persist = async (transaction: any) => {
        ...
        if (sourceChecks.some((check) => check.status === "parser_degraded" || check.status === "rate_limited" || check.status === "hard_failed")) {
          const [item] = await transaction.insert(agentInboxItems).values({ id: deps.id(), userId: run.userId, runId: run.id, triggerEventSequence: terminalSequence, kind: "source_attention", status: "unread", reasonCode: "SOURCE_HEALTH_ATTENTION", budgetDimension: null, createdAt: input.now }).onConflictDoNothing().returning({ id: agentInboxItems.id });
