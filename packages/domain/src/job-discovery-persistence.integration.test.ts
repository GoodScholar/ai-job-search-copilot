...
describe("job discovery persistence lifecycle", () => {
  ...
  async function claimRun(userId: string, targetId: string, now: Date) {
    ...
    const [run] = await database.update(agentRuns).set({ status: "running", currentStep: "persist_results", claimToken, attemptCount: 1, startedAt: now, activeSliceStartedAt: now, claimExpiresAt: new Date(now.getTime() + 30_000) })
  ...
  it("完整空扫描关闭来源时追加生命周期版本并复用上一原始对象", async () => {
    ...
    await persistence.persistSuccessfulDiscovery({
      run: firstRun,
      details: [{ sourceId: "greenhouse:fictional-labs", detailId: "701", company: "Fictional Labs", title: "AI Engineer", location: "Shanghai", postedAt: "2026-08-30T00:00:00.000Z", deadline: null, sourceType: "greenhouse", isOfficial: true, rawPayload: { id: 701, content: "first" } }],
  ...
  it("仅比较来源最新版本：相同内容复用，正文变化和 reopened 生命周期各追加一版", async () => {
    ...
    await database.insert(jobTargetRevisions).values({ id: crypto.randomUUID(), userId, targetId, version: 1, priority: "primary", state: "active", constraints, createdAt: firstSeen });
    const persistence = createJobDiscoveryPersistence({ db: database, id: () => crypto.randomUUID(), auditTrail: createAuditTrail({ db: database, clock: () => later }) });
    const base = { sourceId: "greenhouse:fictional-labs", detailId: "702", company: "Fictional Labs", title: "AI Engineer", location: "Shanghai", postedAt: null, deadline: null, sourceType: "greenhouse", isOfficial: true };
  ...
  it("不完整扫描不关闭，过期详情不写 run result，全部来源关闭后机会关闭", async () => {
    ...
    const details = [
      { sourceId: sourceA, detailId: "703", company: "Fictional Labs", title: "AI Engineer", location: "Shanghai", postedAt: null, deadline: null, sourceType: "greenhouse", isOfficial: true, rawPayload: { source: "a" } },
      { sourceId: sourceB, detailId: "704", company: "Fictional Labs", title: "AI Engineer", location: "Shanghai", postedAt: null, deadline: null, sourceType: "greenhouse", isOfficial: true, rawPayload: { source: "b" } },
    ];
  ...
  it("expired 最新详情标为 expired 且不为该运行写结果", async () => {
    ...
    await persistence.persistSuccessfulDiscovery({ run, details: [{ sourceId: "greenhouse:fictional-labs", detailId: "705", company: "Fictional Labs", title: "Expired role", location: null, postedAt: null, deadline: "2026-08-29T00:00:00.000Z", sourceType: "greenhouse", isOfficial: true, rawPayload: { expired: true } }], scans: [{ sourceId: "greenhouse:fictional-labs", observedDetailIds: ["705"], complete: true }], storedObjects: [{ sourceId: "greenhouse:fictional-labs", detailId: "705", objectKey: "expired.json", rawContentSha256: "f".repeat(64) }], now: later });
  ...
  it("持久化失败回滚详情与完整扫描 reconciliation", async () => {
    ...
    await database.insert(jobTargetRevisions).values({ id: crypto.randomUUID(), userId, targetId, version: 1, priority: "primary", state: "active", constraints, createdAt: firstSeen });
    const persistence = createJobDiscoveryPersistence({ db: database, id: () => crypto.randomUUID(), auditTrail: createAuditTrail({ db: database, clock: () => later }) });
    const detail = { sourceId: "greenhouse:fictional-labs", detailId: "706", company: "Fictional Labs", title: "AI Engineer", location: null, postedAt: null, deadline: null, sourceType: "greenhouse", isOfficial: true, rawPayload: { id: 706 } };
