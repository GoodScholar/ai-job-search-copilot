...
describe("job discovery persistence lifecycle", () => {
  ...
  it("仅接受完全相同的来源健康冲突重放，差异冲突回滚当前终态", async () => {
    ...
    await database.insert(jobTargetRevisions).values({ id: crypto.randomUUID(), userId, targetId, version: 1, priority: "primary", state: "active", constraints, createdAt: firstSeen });
    const persistence = createJobDiscoveryPersistence({ db: database, id: () => crypto.randomUUID(), auditTrail: createAuditTrail({ db: database, clock: () => later }) });
    const check = (runId: string) => ({ checkId: crypto.randomUUID(), runId, targetId, watchlistItemId, sourceId, status: "rate_limited" as const, reasonCodes: ["SOURCE_RATE_LIMITED"] as Array<"SOURCE_RATE_LIMITED">, impact: { scope: "entire_source" as const, affectedCount: null }, observedPostingCount: 0, selectedDetailCount: 0, validDetailCount: 0, requestAttemptCount: 1, checkedAt: later.toISOString() });
    const freezeV3 = async (run: Awaited<ReturnType<typeof claimRun>>) => {
      const [frozen] = await database.update(agentRuns).set({ workflowVersion: GREENHOUSE_SOURCE_HEALTH_WORKFLOW_VERSION, adapterVersion: "greenhouse-job-board-v2", outputSchemaVersion: "job-discovery-result-v3", ruleVersion: "job-discovery-source-health-rules-v1", toolAllowlist: ["job_discovery.list_source", "job_discovery.get_detail"], sourceScope: { kind: "company_watchlist", adapter: "greenhouse", adapterVersion: "greenhouse-job-board-v2", watchlistVersion: 1, sources: [{ sourceId, watchlistItemId, canonicalCompanyName: "Conflict", careersUrl: "https://boards.greenhouse.io/conflict", allowedDomains: ["boards-api.greenhouse.io"], boardToken: "conflict" }] } }).where(eq(agentRuns.id, run.id)).returning();
      if (!frozen) throw new Error("run not frozen");
      return { ...frozen, claimToken: run.claimToken };
    };
    const sameRun = await freezeV3(await claimRun(userId, targetId, later)); const same = check(sameRun.id);
    ...
    await expect(persistence.persistSuccessfulDiscovery({ run: sameRun, details: [], scans: [{ sourceId, observedDetailIds: [], complete: false }], storedObjects: [], sourceChecks: [{ ...same, checkId: crypto.randomUUID() }], now: later })).resolves.toMatchObject({ completed: true });
    ...
    await expect(database.select({ sourceHealthCheckId: agentInboxItems.sourceHealthCheckId, watchlistItemId: agentInboxItems.watchlistItemId, triggerEventSequence: agentInboxItems.triggerEventSequence }).from(agentInboxItems).where(and(eq(agentInboxItems.userId, userId), eq(agentInboxItems.runId, sameRun.id), eq(agentInboxItems.kind, "source_attention")))).resolves.toEqual([{ sourceHealthCheckId: same.checkId, watchlistItemId, triggerEventSequence: null }]);
    const differentRun = await freezeV3(await claimRun(userId, targetId, later)); const existing = check(differentRun.id);
    ...
    await expect(persistence.persistSuccessfulDiscovery({ run: differentRun, details: [], scans: [{ sourceId, observedDetailIds: [], complete: false }], storedObjects: [], sourceChecks: [{ ...existing, checkId: crypto.randomUUID(), requestAttemptCount: 2 }], now: later })).rejects.toThrow("AGENT_RUN_PERSIST_FAILED");
  ...
  it.each(["wrong", "missing", "extra"] as const)("v3 scans 必须与冻结 sourceScope 一一对应：%s", async (variant) => {
    const userId = crypto.randomUUID(); const targetId = crypto.randomUUID(); const watchlistItemId = crypto.randomUUID();
    await database.insert(jobAccounts).values({ id: userId });
    await database.insert(jobTargets).values({ id: targetId, userId, version: 1, priority: "primary", state: "active", activeSlot: null, createdAt: firstSeen, updatedAt: firstSeen });
    await database.insert(jobTargetRevisions).values({ id: crypto.randomUUID(), userId, targetId, version: 1, priority: "primary", state: "active", constraints, createdAt: firstSeen });
    const persistence = createJobDiscoveryPersistence({ db: database, id: () => crypto.randomUUID(), auditTrail: createAuditTrail({ db: database, clock: () => later }) });
    const run = await claimRun(userId, targetId, later);
    const source = { sourceId: "greenhouse:scan_scope", watchlistItemId, canonicalCompanyName: "Scan", careersUrl: "https://boards.greenhouse.io/scan_scope", allowedDomains: ["boards-api.greenhouse.io"], boardToken: "scan_scope" };
    const [frozen] = await database.update(agentRuns).set({ workflowVersion: GREENHOUSE_SOURCE_HEALTH_WORKFLOW_VERSION, adapterVersion: "greenhouse-job-board-v2", outputSchemaVersion: "job-discovery-result-v3", ruleVersion: "job-discovery-source-health-rules-v1", toolAllowlist: ["job_discovery.list_source", "job_discovery.get_detail"], sourceScope: { kind: "company_watchlist", adapter: "greenhouse", adapterVersion: "greenhouse-job-board-v2", watchlistVersion: 1, sources: [source] } }).where(eq(agentRuns.id, run.id)).returning();
    if (!frozen) throw new Error("run not frozen");
    const check = { checkId: crypto.randomUUID(), runId: run.id, targetId, watchlistItemId, sourceId: source.sourceId, status: "zero_valid_results" as const, reasonCodes: [], impact: { scope: "none" as const, affectedCount: null }, observedPostingCount: 0, selectedDetailCount: 0, validDetailCount: 0, requestAttemptCount: 1, checkedAt: later.toISOString() };
    const scans = variant === "wrong" ? [{ sourceId: "greenhouse:other", observedDetailIds: [], complete: true }] : variant === "missing" ? [] : [{ sourceId: source.sourceId, observedDetailIds: [], complete: true }, { sourceId: "greenhouse:other", observedDetailIds: [], complete: true }];
  ...
  it.each(["unknown_source", "illegal_action", "duplicate_source", "zero_count"] as const)("v3 persistence 拒绝畸形 capability issue，且不写终态事实：%s", async (variant) => {
    ...
    const persistence = createJobDiscoveryPersistence({ db: database, id: () => crypto.randomUUID(), auditTrail: createAuditTrail({ db: database, clock: () => later }) });
    const run = await claimRun(userId, targetId, later);
    const denied = { sourceId: "greenhouse:denied", watchlistItemId: deniedWatchlistItemId, canonicalCompanyName: "Denied", careersUrl: "https://boards.greenhouse.io/denied", allowedDomains: ["boards-api.greenhouse.io"], boardToken: "denied" };
    const checked = { sourceId: "greenhouse:checked", watchlistItemId: checkedWatchlistItemId, canonicalCompanyName: "Checked", careersUrl: "https://boards.greenhouse.io/checked", allowedDomains: ["boards-api.greenhouse.io"], boardToken: "checked" };
    const [frozen] = await database.update(agentRuns).set({ workflowVersion: GREENHOUSE_SOURCE_HEALTH_WORKFLOW_VERSION, adapterVersion: "greenhouse-job-board-v2", outputSchemaVersion: "job-discovery-result-v3", ruleVersion: "job-discovery-source-health-rules-v1", toolAllowlist: ["job_discovery.list_source", "job_discovery.get_detail"], sourceScope: { kind: "company_watchlist", adapter: "greenhouse", adapterVersion: "greenhouse-job-board-v2", watchlistVersion: 1, sources: [denied, checked] } }).where(eq(agentRuns.id, run.id)).returning();
    if (!frozen) throw new Error("run not frozen");
    ...
    await expect(database.select({ status: agentRuns.status }).from(agentRuns).where(eq(agentRuns.id, run.id))).resolves.toEqual([{ status: "running" }]);
    await expect(Promise.all([database.select().from(agentRunEvents).where(eq(agentRunEvents.runId, run.id)), database.select().from(agentInboxItems).where(eq(agentInboxItems.runId, run.id)), database.select().from(jobDiscoverySourceIssues).where(eq(jobDiscoverySourceIssues.runId, run.id))])).resolves.toEqual(before);
  });

  it.each(["observed_count", "completion"] as const)("v3 check 必须与同来源 scan 一致：%s", async (variant) => {
    const userId = crypto.randomUUID(); const targetId = crypto.randomUUID(); const watchlistItemId = crypto.randomUUID();
    await database.insert(jobAccounts).values({ id: userId });
    await database.insert(jobTargets).values({ id: targetId, userId, version: 1, priority: "primary", state: "active", activeSlot: null, createdAt: firstSeen, updatedAt: firstSeen });
    await database.insert(jobTargetRevisions).values({ id: crypto.randomUUID(), userId, targetId, version: 1, priority: "primary", state: "active", constraints, createdAt: firstSeen });
    const persistence = createJobDiscoveryPersistence({ db: database, id: () => crypto.randomUUID(), auditTrail: createAuditTrail({ db: database, clock: () => later }) });
    const run = await claimRun(userId, targetId, later); const sourceId = "greenhouse:consistent";
    const [frozen] = await database.update(agentRuns).set({ workflowVersion: GREENHOUSE_SOURCE_HEALTH_WORKFLOW_VERSION, adapterVersion: "greenhouse-job-board-v2", outputSchemaVersion: "job-discovery-result-v3", ruleVersion: "job-discovery-source-health-rules-v1", toolAllowlist: ["job_discovery.list_source", "job_discovery.get_detail"], sourceScope: { kind: "company_watchlist", adapter: "greenhouse", adapterVersion: "greenhouse-job-board-v2", watchlistVersion: 1, sources: [{ sourceId, watchlistItemId, canonicalCompanyName: "Consistent", careersUrl: "https://boards.greenhouse.io/consistent", allowedDomains: ["boards-api.greenhouse.io"], boardToken: "consistent" }] } }).where(eq(agentRuns.id, run.id)).returning();
    if (!frozen) throw new Error("run not frozen");
    const check = { checkId: crypto.randomUUID(), runId: run.id, targetId, watchlistItemId, sourceId, status: "healthy" as const, reasonCodes: [], impact: { scope: "none" as const, affectedCount: null }, observedPostingCount: variant === "observed_count" ? 2 : 1, selectedDetailCount: 1, validDetailCount: 1, requestAttemptCount: 1, checkedAt: later.toISOString() };
  ...
  it.each([
    ["all_zero", ["zero_valid_results"], "completed_with_source_issues"],
    ["zero_and_hard", ["zero_valid_results", "hard_failed"], "completed"],
    ["all_hard", ["hard_failed"], "completed"],
  ] as const)("v3 persistence 拒绝与来源健康事实矛盾的终态：%s", async (_label, statuses, terminal) => {
    const userId = crypto.randomUUID(); const targetId = crypto.randomUUID();
    await database.insert(jobAccounts).values({ id: userId });
    await database.insert(jobTargets).values({ id: targetId, userId, version: 1, priority: "primary", state: "active", activeSlot: null, createdAt: firstSeen, updatedAt: firstSeen });
    await database.insert(jobTargetRevisions).values({ id: crypto.randomUUID(), userId, targetId, version: 1, priority: "primary", state: "active", constraints, createdAt: firstSeen });
    const persistence = createJobDiscoveryPersistence({ db: database, id: () => crypto.randomUUID(), auditTrail: createAuditTrail({ db: database, clock: () => later }) });
    const run = await claimRun(userId, targetId, later);
    const sources = statuses.map((_, index) => ({ sourceId: `greenhouse:terminal_${index}`, watchlistItemId: crypto.randomUUID(), canonicalCompanyName: `Terminal ${index}`, careersUrl: `https://boards.greenhouse.io/terminal_${index}`, allowedDomains: ["boards-api.greenhouse.io"], boardToken: `terminal_${index}` }));
    const [frozen] = await database.update(agentRuns).set({ workflowVersion: GREENHOUSE_SOURCE_HEALTH_WORKFLOW_VERSION, adapterVersion: "greenhouse-job-board-v2", outputSchemaVersion: "job-discovery-result-v3", ruleVersion: "job-discovery-source-health-rules-v1", toolAllowlist: ["job_discovery.list_source", "job_discovery.get_detail"], sourceScope: { kind: "company_watchlist", adapter: "greenhouse", adapterVersion: "greenhouse-job-board-v2", watchlistVersion: 1, sources } }).where(eq(agentRuns.id, run.id)).returning();
    if (!frozen) throw new Error("run not frozen");
    const sourceChecks: import("@job-copilot/contracts/agent-runs").JobSourceHealthCheck[] = sources.map((source, index) => {
      const status = statuses[index]!;
      return { checkId: crypto.randomUUID(), runId: run.id, targetId, watchlistItemId: source.watchlistItemId, sourceId: source.sourceId, status, reasonCodes: status === "hard_failed" ? ["SOURCE_SERVER_ERROR"] : [], impact: status === "hard_failed" ? { scope: "entire_source" as const, affectedCount: null } : { scope: "none" as const, affectedCount: null }, observedPostingCount: 0, selectedDetailCount: 0, validDetailCount: 0, requestAttemptCount: status === "hard_failed" ? 2 : 1, checkedAt: later.toISOString() } as import("@job-copilot/contracts/agent-runs").JobSourceHealthCheck;
    });
    ...
    await expect(persistence.persistSuccessfulDiscovery({ run: { ...frozen, claimToken: run.claimToken }, details: [], scans: sources.map((source, index) => ({ sourceId: source.sourceId, observedDetailIds: [], complete: statuses[index] === "zero_valid_results" })), storedObjects: [], sourceChecks, terminal, now: later })).rejects.toThrow("AGENT_RUN_PERSIST_FAILED");
    await expect(database.select({ status: agentRuns.status }).from(agentRuns).where(eq(agentRuns.id, run.id))).resolves.toEqual([{ status: "running" }]);
  ...
  it("v2 scope 外 complete scan 在没有 sourceChecks 时也拒绝，且不关闭历史 posting", async () => {
    ...
    await database.insert(jobTargetRevisions).values({ id: crypto.randomUUID(), userId, targetId, version: 1, priority: "primary", state: "active", constraints, createdAt: firstSeen });
    const persistence = createJobDiscoveryPersistence({ db: database, id: () => crypto.randomUUID(), auditTrail: createAuditTrail({ db: database, clock: () => later }) });
    await persistence.persistSuccessfulDiscovery({ run: await claimRun(userId, targetId, firstSeen), details: [{ sourceId: outsideId, detailId: "1", company: "Outside", title: "Engineer", location: null, postedAt: null, deadline: null, sourceType: "company_careers", isOfficial: true, rawPayload: {} }], scans: [{ sourceId: outsideId, observedDetailIds: ["1"], complete: true }], storedObjects: [{ sourceId: outsideId, detailId: "1", objectKey: "outside.json", rawContentSha256: "a".repeat(64) }], now: firstSeen });
    const run = await claimRun(userId, targetId, later);
