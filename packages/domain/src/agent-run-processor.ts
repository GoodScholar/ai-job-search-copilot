...
async function persistLayeredPublicOutcome(deps: AgentRunProcessorDependencies, input: {
  ...
  return runTransaction(deps, input.deadline, async (transaction) => {
    ...
    const [run] = await transaction.select().from(agentRuns).where(input.interrupted
      ...
      : and(eq(agentRuns.userId, input.userId), eq(agentRuns.id, input.runId), eq(agentRuns.status, "running"), eq(agentRuns.claimToken, input.claimToken), eq(agentRuns.controlState, "none"), gt(agentRuns.claimExpiresAt, deps.clock())));
    if (!run) return "stale";
  ...
  return {
    async process(job) {
      ...
      const claimed = await runTransaction(deps, deadline, async (transaction) => {
        ...
        await appendBudgetFacts(transaction, { id: deps.id, auditTrail: deps.auditTrail, userId: job.userId, requestId: job.runId, runId: job.runId, version: usageVersion, currentStep: initialStep, usage: agentRunUsageSnapshot(claimedRun), consumed: { activeDurationMs: 0, toolCalls: 0, sourceRequests: 0, modelCalls: 0 }, now: claimNow });
      ...
      try {
        ...
        } else if (claimed.run.workflowVersion !== "deep-match-v1") adapter = deps.adapterResolver.resolve(resolverInput);
      } catch (error) {
      ...
      // The deep-match branch above returns before discovery-only source scopes are read.
      ...
      if (claimed.run.workflowVersion === GREENHOUSE_SOURCE_HEALTH_WORKFLOW_VERSION) {
        ...
        try {
          for (const [index, source] of v3Scope.sources.entries()) {
            ...
            const monitoringAuthorization = authorizeSourceAction({ declaration, action: "continuous_monitoring", expected: { sourceId: source.sourceId, adapter: v3Scope.adapter, adapterVersion: v3Scope.adapterVersion } });
            if (!monitoringAuthorization.allowed) { sourceIssues.push({ provider: "greenhouse", code: monitoringAuthorization.failure.reasonCode, sourceId: source.sourceId, action: "continuous_monitoring", affectedCount: 1 }); continue; }
            const authorization = authorizeSourceAction({ declaration, action: "active_discovery", expected: { sourceId: source.sourceId, adapter: v3Scope.adapter, adapterVersion: v3Scope.adapterVersion } });
            if (!authorization.allowed) { sourceIssues.push({ provider: "greenhouse", code: authorization.failure.reasonCode, sourceId: source.sourceId, action: "active_discovery", affectedCount: 1 }); continue; }
      ...
      for (const result of uniqueSummaries) {
        ...
        const allowed = sourceScope.adapter === "fake" ? sourceScope.sources.includes(detail.data.sourceId) : sourceScope.sources.some((source: { sourceId: string }) => source.sourceId === detail.data.sourceId);
