    ...
    async evaluate(db, request: RunPreflightInput) {
      ...
      const report: RunPreflightReport = RunPreflightReportSchema.parse({
        ...
        items: [
          ...
          { code: "ACCOUNT_RUN_POLICY_READY", severity: "informational", summary: "账户运行策略已就绪", impact: "当前预算和时间窗口允许本次运行。", retryable: false, suggestedActions: [], evidence: { kind: "account_run_policy", revisionNumber: policy.revisionNumber, status: "ready", checkedAt } },
        ],
      });
