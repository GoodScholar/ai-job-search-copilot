...
export type PublicSourceClientConfig = {
...
export function createPublicSourceClient(config: PublicSourceClientConfig): PublicSourceClient {
  ...
  return {
    async get(input) {
      ...
      for (;;) {
        ...
        const hostRelease = await hostSemaphore.acquire(input.signal);
        let response: TransportResponse;
        try {
          response = await requestWithRetry({ current, target, input, startedAt, totalTimeoutMs, transport, sleep, attemptCount });
        } catch (error) {
          throw withAttempts(error, attemptCount === 0 ? 1 : attemptCount);
        } finally {
          hostRelease();
          globalRelease();
        }
        attemptCount = responseAttemptCount(response, attemptCount, input.retry);
        ...
        Object.defineProperty(response, "__finalUrl", { value: current, enumerable: false });
        return response;
      }
    },
  };
...
async function requestWithRetry(args: {
  current: URL; target: { address: string; family: 4 | 6 }; input: Parameters<PublicSourceClient["get"]>[0]; startedAt: number; totalTimeoutMs: number; transport: Transport; sleep: (milliseconds: number) => Promise<void>; attemptCount: number;
}): Promise<TransportResponse> {
  let attempts = args.attemptCount;
  for (;;) {
    ...
    Object.defineProperty(response, "__attemptCount", { value: attempts, enumerable: false });
    if (args.input.retry !== "bounded" || attempts >= MAX_ATTEMPTS || (response.status !== 429 && response.status < 500)) return response;
...
function responseAttemptCount(response: TransportResponse, fallback: number, retry: "none" | "bounded"): number {
  const value = (response as TransportResponse & { __attemptCount?: number }).__attemptCount;
  return value ?? (retry === "none" ? fallback + 1 : Math.max(fallback + 1, 1));
}
...
function isAuthorizedUrl(url: URL, allowedDomains: readonly string[], exactHosts: ReadonlySet<string>, testOrigin: URL | undefined): boolean {
  ...
  const isControlledTestUrl = testOrigin?.origin === url.origin;
