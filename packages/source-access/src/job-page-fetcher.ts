...
function extractJobPage(rawHtml: string, finalUrl: URL): { visibleText: string; canonicalUrl: string } {
  ...
  if ((h1Texts.length === 0 && headingCount >= 2) || (/(?:engineering\s+jobs|open\s+positions|职位列表|招聘岗位)/iu.test(title ?? "") && h2Count >= 2)) throw new JobPageFetchError("JOB_PAGE_LISTING");
  ...
  if (!title || jobContextSignals.filter((signal) => signal.test(visibleText)).length < 2) throw new JobPageFetchError("JOB_PAGE_UNRECOGNIZED");
  let canonicalUrl = finalUrl.toString();
  if (canonical) {
    try {
      const parsed = new URL(canonical, finalUrl);
      if (/^https?:$/u.test(parsed.protocol) && normalizedHostname(finalUrl.hostname) === normalizedHostname(parsed.hostname)) canonicalUrl = parsed.toString();
