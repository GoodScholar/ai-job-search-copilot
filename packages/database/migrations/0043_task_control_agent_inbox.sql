...
WITH source_items AS (
  SELECT "id", "user_id", "run_id", row_number() OVER (PARTITION BY "run_id" ORDER BY "created_at", "id") AS ordinal
  FROM "agent_inbox_items"
  WHERE "kind" = 'source_attention'
), source_checks AS (
  SELECT "user_id", "run_id", "watchlist_item_id", row_number() OVER (PARTITION BY "run_id" ORDER BY "checked_at", "id") AS ordinal
  FROM "job_source_health_checks"
)
UPDATE "agent_inbox_items" AS item
SET "watchlist_item_id" = source_checks."watchlist_item_id"
FROM source_items
JOIN source_checks ON source_checks."user_id" = source_items."user_id" AND source_checks."run_id" = source_items."run_id" AND source_checks.ordinal = source_items.ordinal
WHERE item."id" = source_items."id";--> statement-breakpoint
UPDATE "agent_inbox_items"
SET "kind" = 'discovery_attention', "reason_code" = 'DISCOVERY_ATTENTION'
WHERE "kind" = 'source_attention' AND "watchlist_item_id" IS NULL;--> statement-breakpoint
...
ALTER TABLE "agent_inbox_items" ADD CONSTRAINT "agent_inbox_items_reference_combination_check" CHECK (("kind" in ('run_failed', 'budget_exhausted', 'decision_required', 'discovery_attention') and "run_id" is not null and "trigger_event_sequence" is not null and "candidate_fact_id" is null and "watchlist_item_id" is null and "recommendation_list_id" is null and "calibration_proposal_id" is null) or ("kind" = 'source_attention' and "run_id" is not null and "watchlist_item_id" is not null and "candidate_fact_id" is null and "recommendation_list_id" is null and "calibration_proposal_id" is null) or ("kind" = 'candidate_fact' and "run_id" is null and "trigger_event_sequence" is null and "candidate_fact_id" is not null and "watchlist_item_id" is null and "recommendation_list_id" is null and "calibration_proposal_id" is null) or ("kind" = 'recommendation_list' and "run_id" is null and "trigger_event_sequence" is null and "candidate_fact_id" is null and "watchlist_item_id" is null and "recommendation_list_id" is not null and "calibration_proposal_id" is null) or ("kind" = 'calibration_proposal' and "run_id" is null and "trigger_event_sequence" is null and "candidate_fact_id" is null and "watchlist_item_id" is null and "recommendation_list_id" is null and "calibration_proposal_id" is not null));--> statement-breakpoint
...
DO $$ BEGIN
  ...
  END IF;
END $$;--> statement-breakpoint
ALTER TABLE "job_source_health_checks" ADD CONSTRAINT "job_source_health_checks_user_run_watchlist_unique" UNIQUE ("user_id", "run_id", "watchlist_item_id");--> statement-breakpoint
ALTER TABLE "agent_inbox_items" ADD CONSTRAINT "agent_inbox_items_owner_source_health_check_fk" FOREIGN KEY ("user_id", "run_id", "watchlist_item_id") REFERENCES "public"."job_source_health_checks"("user_id", "run_id", "watchlist_item_id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "agent_inbox_items_source_run_watchlist_unique_idx" ON "agent_inbox_items" USING btree ("run_id", "watchlist_item_id") WHERE "kind" = 'source_attention';--> statement-breakpoint
