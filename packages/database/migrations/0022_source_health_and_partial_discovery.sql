...
CREATE TABLE "job_source_health_checks" (
	...
	CONSTRAINT "job_source_health_checks_status_evidence_check" CHECK (
	  ...
	  or ("job_source_health_checks"."status" = 'parser_degraded' and "job_source_health_checks"."reason_codes" ?| array['SOURCE_LIST_SCHEMA_INVALID', 'SOURCE_DETAIL_FIELDS_MISSING', 'SOURCE_DETAIL_URL_INVALID', 'SOURCE_DETAIL_IDENTITY_INVALID'])
	  or ("job_source_health_checks"."status" = 'rate_limited' and "job_source_health_checks"."reason_codes" = '["SOURCE_RATE_LIMITED"]'::jsonb)
	  or ("job_source_health_checks"."status" = 'hard_failed' and "job_source_health_checks"."reason_codes" ?| array['SOURCE_AUTH_FAILED', 'SOURCE_TIMEOUT', 'SOURCE_UNREACHABLE', 'SOURCE_SERVER_ERROR', 'SOURCE_POLICY_REJECTED'])
	)
);
