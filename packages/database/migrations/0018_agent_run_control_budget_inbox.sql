...
ALTER TABLE "agent_runs" ADD CONSTRAINT "agent_runs_execution_claim_check" CHECK (("agent_runs"."claim_token" is null and "agent_runs"."claim_expires_at" is null and "agent_runs"."active_slice_started_at" is null) or ("agent_runs"."claim_token" is not null and "agent_runs"."claim_expires_at" is not null and "agent_runs"."active_slice_started_at" is not null and "agent_runs"."status" = 'running'));--> statement-breakpoint
