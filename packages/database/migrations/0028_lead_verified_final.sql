  ...
  IF EXISTS (
    ...
    WHERE "state" = 'verified'
      AND (
        ...
        OR "verified_final_url" ~* '^https://(?:[a-z0-9-]+\.)*(?:localhost|invalid|test|example|onion|arpa)(?::|/|\?|$)'
      )
...
DO $$ BEGIN
  IF EXISTS (
    ...
    WHERE posting."source_identity" ->> 'canonicalUrl' IS NULL
      ...
      OR posting."source_identity" ->> 'canonicalUrl' ~* '^https://(?:[a-z0-9-]+\.)*(?:localhost|invalid|test|example|onion|arpa)(?::|/|\?|$)'
