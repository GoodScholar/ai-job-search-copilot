...
describe("database migrations", () => {
  ...
  it("registers the 0047 preflight snapshot migration before 0048", async () => {
    ...
    expect(journal.entries.at(-3)).toEqual({ idx: 47, version: "7", when: expect.any(Number), tag: "0047_agent_run_preflight_snapshot", breakpoints: true });
    expect(journal.entries.at(-2)).toEqual({ idx: 48, version: "7", when: 1788685000000, tag: "0048_first_recommendation_journey", breakpoints: true });
    expect(journal.entries.at(-1)).toEqual({ idx: 49, version: "7", when: 1789200000000, tag: "0049_recommendation_runs", breakpoints: true });
  });
