...
describe("账户运行停止控制迁移", () => {
  ...
  it("强制账户控制版本、严格快照、账户内幂等键和不可变命令", async () => {
    ...
    for (const invalidSnapshot of [JSON.stringify({ applied: false }), JSON.stringify({ applied: "false", state: { stoppedAt: null, controlVersion: 0, scheduleResumeAfter: null } }), JSON.stringify({ applied: false, state: { stoppedAt: null, controlVersion: "0", scheduleResumeAfter: null } }), JSON.stringify({ applied: false, state: { stoppedAt: null, controlVersion: 0, scheduleResumeAfter: null, padding: "x".repeat(2_048) } })]) {
