...
export const jobDiscoveryLeads = pgTable("job_discovery_leads", {
  ...
  check("job_discovery_leads_verified_final_url_safe_check", sql`${table.verifiedFinalUrl} is null or (${table.verifiedFinalUrl} ~* '^https://[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)+(?:\\:[1-9][0-9]{0,4})?(?:/[^?#]*)?(?:\\?(?:(?:id|job|jobid|job_id|openingid|opening_id|positionid|position_id|requisitionid|requisition_id)=[A-Za-z0-9._~-]{1,128}(?:&(?:id|job|jobid|job_id|openingid|opening_id|positionid|position_id|requisitionid|requisition_id)=[A-Za-z0-9._~-]{1,128})*)?)?$' and ${table.verifiedFinalUrl} !~* '^https://(?:[0-9]{1,3}\\.){3}[0-9]{1,3}(?::|/|\\?|$)' and ${table.verifiedFinalUrl} !~* '^https://(?:[a-z0-9-]+\\.)*(?:localhost|invalid|test|example|onion|arpa)(?::|/|\\?|$)')`),
