...
export const agentInboxItems = pgTable("agent_inbox_items", {
  ...
  scheduleOccurrenceId: uuid("schedule_occurrence_id"),
  ...
  check("agent_inbox_items_kind_check", sql`${table.kind} in ('run_failed', 'budget_exhausted', 'decision_required', 'source_attention', 'discovery_attention', 'candidate_fact', 'recommendation_list', 'recommendation_result', 'calibration_proposal', 'schedule_attention')`),
  check("agent_inbox_items_status_check", sql`${table.status} in ('unread', 'read', 'resolved')`),
  check("agent_inbox_items_reason_check", sql`${table.reasonCode} in ('AGENT_RUN_PAUSED', 'SOURCE_HEALTH_ATTENTION', 'DISCOVERY_ATTENTION', 'CANDIDATE_FACT_PENDING', 'RECOMMENDATION_LIST_PUBLISHED', 'NO_RECOMMENDATIONS_PUBLISHED', 'CALIBRATION_PROPOSAL_CREATED', 'SCHEDULE_RUN_PREFLIGHT_BLOCKED', 'AGENT_RUN_ADAPTER_RETRYABLE', 'AGENT_RUN_ADAPTER_FAILED', 'AGENT_RUN_CONTENT_STORAGE_FAILED', 'AGENT_RUN_PERSIST_FAILED', 'AGENT_RUN_BUDGET_EXCEEDED', 'AGENT_RUN_MODEL_RETRYABLE', 'AGENT_RUN_MODEL_AUTH_FAILED', 'AGENT_RUN_MODEL_POLICY_REJECTED', 'AGENT_RUN_MODEL_INVALID_RESPONSE')`),
  ...
  check("agent_inbox_items_reference_combination_check", sql`
    (${table.kind} in ('run_failed', 'budget_exhausted', 'decision_required', 'discovery_attention') and ${table.runId} is not null and ${table.triggerEventSequence} is not null and ${table.scheduleOccurrenceId} is null and ${table.candidateFactId} is null and ${table.watchlistItemId} is null and ${table.sourceHealthCheckId} is null and ${table.recommendationListId} is null and ${table.recommendationResultId} is null and ${table.calibrationProposalId} is null)
    or (${table.kind} = 'source_attention' and ${table.runId} is not null and ${table.scheduleOccurrenceId} is null and ${table.watchlistItemId} is not null and ${table.sourceHealthCheckId} is not null and ${table.candidateFactId} is null and ${table.recommendationListId} is null and ${table.recommendationResultId} is null and ${table.calibrationProposalId} is null)
    or (${table.kind} = 'candidate_fact' and ${table.runId} is null and ${table.triggerEventSequence} is null and ${table.scheduleOccurrenceId} is null and ${table.candidateFactId} is not null and ${table.watchlistItemId} is null and ${table.sourceHealthCheckId} is null and ${table.recommendationListId} is null and ${table.recommendationResultId} is null and ${table.calibrationProposalId} is null)
    or (${table.kind} = 'recommendation_list' and ${table.runId} is null and ${table.triggerEventSequence} is null and ${table.scheduleOccurrenceId} is null and ${table.candidateFactId} is null and ${table.watchlistItemId} is null and ${table.sourceHealthCheckId} is null and ${table.recommendationListId} is not null and ${table.recommendationResultId} is null and ${table.calibrationProposalId} is null)
    or (${table.kind} = 'recommendation_result' and ${table.runId} is null and ${table.triggerEventSequence} is null and ${table.scheduleOccurrenceId} is null and ${table.candidateFactId} is null and ${table.watchlistItemId} is null and ${table.sourceHealthCheckId} is null and ${table.recommendationListId} is null and ${table.recommendationResultId} is not null and ${table.calibrationProposalId} is null)
    or (${table.kind} = 'calibration_proposal' and ${table.runId} is null and ${table.triggerEventSequence} is null and ${table.scheduleOccurrenceId} is null and ${table.candidateFactId} is null and ${table.watchlistItemId} is null and ${table.sourceHealthCheckId} is null and ${table.recommendationListId} is null and ${table.recommendationResultId} is null and ${table.calibrationProposalId} is not null)
    or (${table.kind} = 'schedule_attention' and ${table.runId} is null and ${table.triggerEventSequence} is null and ${table.scheduleOccurrenceId} is not null and ${table.candidateFactId} is null and ${table.watchlistItemId} is null and ${table.sourceHealthCheckId} is null and ${table.recommendationListId} is null and ${table.recommendationResultId} is null and ${table.calibrationProposalId} is null)
  `),
]);
