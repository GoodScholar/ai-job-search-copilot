...
export const calibrationProposalRevisions = pgTable("calibration_proposal_revisions", {
  id: uuid("id").primaryKey().defaultRandom(), userId: uuid("user_id").notNull().references(() => jobAccounts.id), proposalId: uuid("proposal_id").notNull().references(() => calibrationProposals.id), revisionNumber: integer("revision_number").notNull(), strategy: varchar("strategy", { length: 48 }).notNull(), ruleConfig: jsonb("rule_config").notNull(), impactPreview: jsonb("impact_preview").notNull(), idempotencyKey: uuid("idempotency_key").notNull(), commandSummary: varchar("command_summary", { length: 64 }).notNull(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
