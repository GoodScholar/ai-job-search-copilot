...
}, (table) => [unique("recommendation_decision_responses_event_unique").on(table.decisionEventId), foreignKey({ columns: [table.userId, table.decisionEventId], foreignColumns: [recommendationDecisionEvents.userId, recommendationDecisionEvents.id], name: "recommendation_decision_responses_owner_event_fk" })]);
