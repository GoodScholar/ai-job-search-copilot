...
@Module({
  ...
  providers: [{
    ...
    useFactory: (db: Database, runs: AgentRunCommands, auditTrail: AuditTrail) => createJobDiscoverySchedules({ db, runs: runs as AgentRunStarter, auditTrail, id: () => crypto.randomUUID(), clock: () => new Date(), executionMode: createConfiguredJobDiscoveryExecutionMode() }),
  }],
})
