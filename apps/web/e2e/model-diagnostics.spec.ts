...
async function resetModelDiagnosticProjection() {
  const client = new Client({ connectionString: databaseUrl });
  await client.connect();
  try {
    await client.query("delete from model_diagnostic_results");
  } finally {
    await client.end();
  }
}
...
async function enterModelConnection(page: Page) {
  ...
  await page.getByLabel("运行前检查").getByRole("link", { name: "检查模型连接" }).click();
...
test("从运行设置进入模型连接页并以受控 Fake 展示稳定状态", async ({ page, request }, testInfo) => {
  ...
  await resetModelDiagnosticProjection();
  await enterModelConnection(page);
  const status = page.getByRole("status");
  await expect(status).toContainText("尚未完成模型连接检查");
