  ...
  return (
    <section aria-labelledby="agent-run-title" className={`workbench-ledger agent-run-panel${runIsUnfinished ? " agent-run-panel-active" : ""}`} id="agent-run">
      ...
      {run && "sourceIssues" in run && run.sourceIssues.some((issue) => "impact" in issue && (issue.code === "SOURCE_CAPABILITY_UNSUPPORTED" || issue.code === "SOURCE_CAPABILITY_DECLARATION_MISMATCH")) ? (() => {
        ...
        const affectedCount = capabilityIssues.reduce((total, issue) => total + issue.affectedCount, 0);
        const affectedLabel = capabilityIssues.some((issue) => issue.affectedCount === 10) ? `至少 ${affectedCount}` : String(affectedCount);
        const reason = capabilityIssues.map((issue) => issue.code === "SOURCE_CAPABILITY_UNSUPPORTED" ? "该来源未声明所需能力" : "该来源声明与本次冻结执行规格不一致").join("；");
        const deniedNames = "sourceChecks" in run ? run.sourceScope.sources.filter((source): source is Exclude<typeof source, string> => typeof source !== "string" && !run.sourceChecks.some((check) => check.sourceId === source.sourceId)).map((source) => source.canonicalCompanyName) : [];
        return <p>来源能力拒绝：{reason}；受影响来源 {affectedLabel} 个{deniedNames.length > 0 ? `（${deniedNames.join("、")}）` : ""}；影响范围：{capabilityIssues.map((issue) => issue.impact.scope === "entire_source" ? "整个来源" : "受限范围").join("、")}；{capabilityIssues.every((issue) => issue.retryable === false) ? "不可重试" : "可重试"}。建议：{capabilityIssues.flatMap((issue) => issue.suggestedActions).map((action) => action === "review_source_capabilities" ? "检查来源能力声明" : action).join("、")}。 <Link className="workbench-touch-target" href={`/profile/targets/${run.targetId}/watchlist#source-capabilities`}>查看来源能力</Link></p>;
