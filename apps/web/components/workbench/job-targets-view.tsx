...
export function JobTargetsView({ initialOverview }: { initialOverview: JobTargetOverview }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = target ? { expectedVersion: target.version, priority, constraints } : { priority, constraints };
    try {
  ...
  return <main className="container profile-main job-targets-main">
    ...
    <section aria-labelledby="job-target-form-title" className="job-targets-section"><h2 id="job-target-form-title">{editingTarget ? `修改 ${editingTarget.constraints.roleFamily}` : "手动确认求职目标"}</h2><form className="job-target-form" onSubmit={submit}>
      <fieldset><legend>目标优先级</legend><label><input checked={priority === "primary"} disabled={primaryUnavailable} name="priority" onChange={() => setPriority("primary")} type="radio" value="primary" /> 主目标</label><label><input checked={priority === "secondary"} disabled={secondaryUnavailable} name="priority" onChange={() => setPriority("secondary")} type="radio" value="secondary" /> 次目标</label></fieldset>
      ...
      <button className="profile-upload-button workbench-touch-target" disabled={isSaving} type="submit">{editingTarget ? "保存修改" : priority === "primary" ? "保存主目标" : "保存次目标"}</button>
