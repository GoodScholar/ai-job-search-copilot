...
function WorkbenchHomeContent({ home, targets, initialRun, initialRecommendationPreparation = null, initialRecommendationRun = null, preflight: initialPreflight = null, inbox, unavailableSections = [] }: WorkbenchHomeViewProps) {
  ...
  return (
    <main className="container workbench-main">
      ...
      <RecommendationRunPanel initialPreparation={initialRecommendationPreparation} initialRun={initialRecommendationRun} key={`${initialRecommendationRun?.runId ?? "new"}:${initialRecommendationPreparation?.preflight.checkedAt ?? "unavailable"}`} onRunChanged={() => router.refresh()} unavailable={recommendationUnavailable} />
