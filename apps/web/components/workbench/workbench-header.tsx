  ...
  return (
    <header className="workbench-header">
      <div className="container workbench-header-inner">
        <Link className="workbench-brand" href="/home">AI Job Search Copilot</Link>
        <nav aria-label="求职工作台导航" className="workbench-nav">
          {navigation.map((item) => item === "首页" ? (
            <Link aria-current="page" className="workbench-nav-link" href="/home" key={item}>{item}</Link>
          ) : (
            <span aria-disabled="true" className="workbench-nav-pending" key={item}>{item}</span>
          ))}
        </nav>
        <form action={submitEndSession}>
          <Button className="workbench-signout" size="lg" type="submit" variant="outline">退出</Button>
        </form>
      </div>
    </header>
  );
