import { DeactivateJobTargetCommandSchema } from "@job-copilot/contracts/job-targets";
import { z } from "zod";
...
export async function POST(request: Request, { params }: RouteContext): Promise<Response> {
  ...
  const body = DeactivateJobTargetCommandSchema.safeParse(await request.json().catch(() => null));
  if (!body.success) return emptyResponse(400);
  try {
    return Response.json(await api.deactivateJobTarget(sessionToken, targetId, body.data), { status: 201, headers: noStore });
  } catch (error) {
