import { CompanyWatchlistOverviewSchema, SetCompanyWatchlistItemStateCommandSchema } from "@job-copilot/contracts/company-watchlists";
...
export async function POST(request: Request, { params }: { params: Promise<{ targetId: string; itemId: string }> }): Promise<Response> {
  ...
  const command = SetCompanyWatchlistItemStateCommandSchema.safeParse(await request.json().catch(() => null));
  if (!command.success) return problem(400);
  try {
    return Response.json(CompanyWatchlistOverviewSchema.parse(await api.setCompanyWatchlistItemState(sessionToken, targetId, itemId, command.data)), { status: 201, headers: noStore });
  } catch (error) {
