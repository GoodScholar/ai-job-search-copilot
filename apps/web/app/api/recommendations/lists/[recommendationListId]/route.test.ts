...
it("精确清单代理成功、401 与未知上游，并保持 no-store", async () => {
  ...
  mocks.getRecommendationList.mockRejectedValue(new Error("secret")); const unknown = await GET(new Request(`http://localhost/api/recommendations/lists/${listId}?targetId=${targetId}`), { params: Promise.resolve({ recommendationListId: listId }) });
