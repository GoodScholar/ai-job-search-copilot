...
it("精确清单代理成功、401、畸形成功响应与普通未知错误，并保持 no-store", async () => {
  ...
  mocks.getRecommendationList.mockRejectedValue({ status: 200, kind: "invalid_response" }); const unknown = await GET(new Request(`http://localhost/api/recommendations/lists/${listId}?targetId=${targetId}`), { params: Promise.resolve({ recommendationListId: listId }) }); expect(await unknown.text()).toBe("");
  mocks.getRecommendationList.mockRejectedValue(new Error("unknown")); const ordinaryUnknown = await GET(new Request(`http://localhost/api/recommendations/lists/${listId}?targetId=${targetId}`), { params: Promise.resolve({ recommendationListId: listId }) }); expect(ordinaryUnknown.status).toBe(502); expect(await ordinaryUnknown.text()).toBe("");
  ...
  expect([unauthorized.status, unknown.status, localUnauthorized.status]).toEqual([401, 502, 401]); for (const response of [success, unauthorized, unknown, ordinaryUnknown, localUnauthorized]) expect(response.headers.get("cache-control")).toBe("no-store");
});
