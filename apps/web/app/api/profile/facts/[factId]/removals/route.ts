import { RemoveProfileFactCommandSchema } from "@job-copilot/contracts/profile-review";
import { z } from "zod";
...
export async function POST(request: Request, { params }: RouteContext): Promise<Response> {
  ...
  const body = RemoveProfileFactCommandSchema.safeParse(await request.json().catch(() => null));
  if (!body.success) return emptyResponse(400);
  try {
    const profile = await api.removeProfileFact(sessionToken, factId, body.data);
