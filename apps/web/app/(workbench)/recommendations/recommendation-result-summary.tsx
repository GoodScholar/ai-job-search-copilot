...
export function RecommendationResultSummary({ result }: { result: RecommendationResult }) {
  ...
  return <section aria-labelledby="recommendation-result-title" className="recommendation-result-summary" id="recommendation-result">
    ...
    <section aria-label="本次覆盖证据" className="recommendation-result-evidence">
      <p>来源覆盖：计划可信来源 {evidence.sourceCoverage.plannedTrustedSourceCount} 个、公开查询 {evidence.sourceCoverage.plannedPublicQueryCount} 个；已检查 {evidence.sourceCoverage.checkedBranchCount} 项，可信 {evidence.sourceCoverage.credibleBranchCount} 项；已验证并发现岗位 {evidence.sourceCoverage.verifiedJobCount} 个{evidence.sourceCoverage.rootBudgetExcludedJobCount > 0 ? `；其中 ${evidence.sourceCoverage.rootBudgetExcludedJobCount} 个因本次结果限额未进入资格筛选。` : "。"}</p>
