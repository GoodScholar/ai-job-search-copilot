...
export function CalibrationProposals({ proposals, reviseAction, resolveAction }: { proposals: CalibrationProposal[]; reviseAction: (proposalId: string, formData: FormData) => Promise<void>; resolveAction: (proposalId: string, formData: FormData) => Promise<void> }) {
  ...
  const [idempotencyKeys] = useState(() => new Map(proposals.filter((proposal) => proposal.status === "pending").flatMap((proposal) => [[`${proposal.proposalId}:revision`, crypto.randomUUID()], [`${proposal.proposalId}:resolution`, crypto.randomUUID()]])));
  ...
  const keyFor = (key: string) => idempotencyKeys.get(key) ?? "";
