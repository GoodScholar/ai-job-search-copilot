...
export class AnySearchPublicJobAdapter {
  ...
  private async request(path: "/v1/search" | "/v1/extract", body: Record<string, unknown>, signal?: AbortSignal): Promise<AnySearchResult<unknown>> {
    ...
    try {
      ...
      try {
        response = await abortable(this.fetch(new URL(path, this.baseUrl), { method: "POST", headers: { authorization: "x".repeat(this.apiKey.length + 7), "content-type": "application/json" }, body: JSON.stringify(body), redirect: "manual", signal: controller.signal }), controller.signal);
      } catch {
        if (signal?.aborted) return error("ANYSEARCH_CANCELLED", false, null);
