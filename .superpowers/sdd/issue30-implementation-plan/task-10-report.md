...
本轮从 `175d8ff` 接续，未创建 worktree、分支、PR、push 或 merge。严格 TDD 提交为 recovered-base Red `46f7510` → Green `7f2a61f`，shared-policy Red `32e15ec` → Green `bb9b423`；最小 lockfile 为 `1857a02`，两条直接裁决的 E2E 说明为 `a86515f`。

恢复路径的 Red 在 Worker module seam 以新鲜 AnySearch adapter（即无进程内 issued-candidate Map）重建一条 claim-bound v4 candidate；受控 `fetch` 仅接受固定 local fixture origin。Red 因 helper 尚不存在而在该 seam 失败（`/tmp/issue30-slice9-fix1-recovered-red.log`），Green 以同一命令通过 **37/37**（`/tmp/issue30-slice9-fix1-recovered-green.log`）。Green 只把已经由 domain runtime config 验证的 fixed base 交给 exact configured phase 的首个与恢复 adapter；missing-key、local 与 production 不获得测试 base，恢复授权仍仅是单 candidate/claim-bound 闭包。
