...
本轮以 `ec4fa535d4be62e110f890031ee6ef2997dda570` 为起点。每个 Red 都在保持其余前置断言 Green 的情况下，到达对应的公开断言；所有失败输出仅含固定字面量或布尔值，不含 raw query、URL、source identity JSON、用户事实或凭据。每次测试前确认无遗留 pnpm、Vitest、Playwright、tsc、Drizzle、E2E runner 或 local-runtime 进程，命令严格单进程串行。
...
| `d8fe6ac` → `e64a260` | Red 只在 missing-key 断言使用的 test-only facts 副本内加入固定非秘密 placeholder，运行时、返回 run、数据库和日志均未写入该值；两个浏览器都在完整 `{ run, facts }` 的 includes 布尔断言得到 `true` 后失败。Green 删除 probe，恢复对真实 returned run 与 persisted facts 的完整排除检查。 | `/tmp/issue30-slice9-round4-secret-exclusion-red.log`、`/tmp/issue30-slice9-round4-e2e-final-green.log` |

最终 Green 的完整配置阶段 + missing-key 阶段在 Desktop Chrome、Mobile Safari 均通过，合计 **4/4**（`/tmp/issue30-slice9-round4-e2e-final-green.log`）。相关回归 `pnpm --filter domain exec vitest run src/agent-run-processor.integration.test.ts --no-file-parallelism` 为 **62/62**（`/tmp/issue30-slice9-round4-agent-run-processor-green.log`）；domain、worker、web typecheck 全通过（`/tmp/issue30-slice9-round4-domain-typecheck.log`、`/tmp/issue30-slice9-round4-worker-typecheck.log`、`/tmp/issue30-slice9-round4-web-typecheck.log`）；Drizzle check 为 `Everything's fine`（`/tmp/issue30-slice9-round4-drizzle-check.log`）。E2E 生成的 `apps/api/dist` 与 `apps/worker/dist` 已按精确路径清理，未纳入提交。
