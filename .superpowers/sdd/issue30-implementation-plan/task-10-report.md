...
| `7d953c2` → `84e4e59` | 替代 Red 将 fixed placeholder 以分段 helper 构造，只在 test-only facts 副本内注入；两个浏览器均在完整 `{ run, facts }` helper 的 boolean 断言得到 `true` 后失败。失败源码上下文只含无敏感字面量的 helper/变量名。Green 删除 probe，恢复真实 returned run 与 persisted facts 的完整排除检查。 | `/tmp/issue30-slice9-round4-secret-exclusion-red-safe.log`、`/tmp/issue30-slice9-round4-secret-exclusion-green-safe.log` |

最终 Green 的完整配置阶段 + missing-key 阶段在 Desktop Chrome、Mobile Safari 均通过，合计 **4/4**（`/tmp/issue30-slice9-round4-secret-exclusion-green-safe.log`）。对替代 Red 运行 `rg -n -e 'fake-anysearch-public-job-test-key' -e 'https?://boards\\.greenhouse\\.io' -e 'source_identity' -e 'raw query' /tmp/issue30-slice9-round4-secret-exclusion-red-safe.log`，结果为 **0 行**（`/tmp/issue30-slice9-round4-secret-red-safety-scan.log`）。相关回归 `pnpm --filter domain exec vitest run src/agent-run-processor.integration.test.ts --no-file-parallelism` 为 **62/62**（`/tmp/issue30-slice9-round4-agent-run-processor-green.log`）；domain、worker、web typecheck 全通过（`/tmp/issue30-slice9-round4-domain-typecheck.log`、`/tmp/issue30-slice9-round4-worker-typecheck.log`、`/tmp/issue30-slice9-round4-web-typecheck.log`）；Drizzle check 为 `Everything's fine`（`/tmp/issue30-slice9-round4-drizzle-check.log`）。E2E 生成的 `apps/api/dist` 与 `apps/worker/dist` 已按精确路径清理，未纳入提交。
