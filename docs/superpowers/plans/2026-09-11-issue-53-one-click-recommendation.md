...
- 同键同义返回原 root 和原响应；同键异义稳定 409。不同键只在没有既有 command 时复用同账户活动 root 并写 alias。活动定义为 `purpose = recommendation`、尚无 immutable result，且 root 或唯一 child 仍为 queued/running/paused；failed/cancelled 或已有结果后允许新 root。
...
- `control` 在同一账户锁事务内确定当前 physical root/child，先读 Task 2 逻辑命令事实。若已存在，比较 fingerprint 后直接返回其不可变响应；若不存在，调用 transaction-bound physical control seam并记录 `physical_run_id`。跨阶段 replay 不得重新作用于 child；物理阶段恰好切换且无法满足命令时返回稳定 409 并要求客户端刷新 authoritative projection。
