...
`(user_id, idempotency_key)` 唯一并绑定同账户根运行。同键同命令不重复执行，返回原运行的最新逻辑投影并标记 `reused: true`；同键异义返回稳定冲突。不新增首次完整响应快照存储（2026-09-13 用户确认）。
...
逻辑控制命令需要以 `(user_id, root_run_id, command_id)` 保持幂等，并记录命令实际作用的物理运行。相同命令在阶段切换后重放不得错误作用于另一个物理运行。重放保留首次命令的 `applied` 事实，但 `run` 返回原逻辑运行的最新投影；既有不可变物理快照用于保存首次作用事实，不作为当前逻辑状态返回，不新增完整逻辑响应快照（2026-09-13 用户确认）。
